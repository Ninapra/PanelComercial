...
# Archivos que contienen patrones literales por diseño (fuente de verdad de
# reglas, allowlists, plantillas con placeholders). Sus hunks se excluyen
# del escaneo para evitar autodetección.
...
def main() -> int:
    ...
    for ln in diff.splitlines():
        header = DIFF_FILE_HEADER.match(ln)
        if header:
            current_file = header.group(1).replace("\\", "/")
            continue
        if current_file in PATTERN_SOURCE_FILES:
            continue
